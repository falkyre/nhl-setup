...
def preferences_settings(default_config,qmark):
    ...
    location = [
        {
            ...
            'default': get_default_value(default_config,['preferences','location'],"string") or ''
        }
    ]
    ...
    selected_teams = get_default_value(default_config,['preferences','teams'],"string") or []
    preferences_teams = []
...
def states_settings(default_config,qmark,setup_type,config_dir):
    ...
    states_config = get_default_value(default_config,['states'],"string") or {}
...
def board_settings(default_config,qmark,setup_type):
    ...
    for aboard in theboards:
        if aboard == 'scoreticker':
            boards_config['scoreticker'] = scoreticker(default_config,qmark) or {}
        if aboard == 'seriesticker':
            boards_config['seriesticker'] = seriesticker(default_config,qmark) or {}
        if aboard == 'standings':
            boards_config['standings'] = standings(default_config,qmark) or {}
        if aboard == 'clock':
            boards_config['clock'] = clock(default_config,qmark) or {}
            #boards_config.update(clock(default_config,qmark))
        if aboard == 'weather':
            boards_config['weather'] = weather(default_config,qmark) or {}
        if aboard == 'wxalert':
            boards_config['wxalert'] = wxalert(default_config,qmark) or {}
